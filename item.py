...
class CustomItem(ctk.CTkFrame):
    def __init__(self, parent, product_object, flag):
        ...
        self.img_label = tk.Label(
            self, image=self.img, width=120, height=120, bg="white")
        self.img_label.place(x=0, y=0)
        self.name_label = tk.Label(self, text=product_object.product_name, font=(
            "Arial", 20), bg="white", fg="red")
        self.name_label.place(x=130, y=10)
        if flag:
            self.delete_button = ctk.CTkButton(
                self, text="Delete", width=10, fg_color='#57A1F8', bg_color="white",text_color="white", border_width=0, command=self.delete_item)
            self.delete_button.place(x=340, y=15)

        self.description_label = tk.Label(
            self, text=product_object.description, font=("Arial", 10), bg="white")
        self.description_label.place(x=130, y=50)
        self.phone_label = tk.Label(self, text=get_phone(
            product_object.user_id), font=("Arial", 10), bg="white")
...
class ItemListPage(ctk.CTkFrame):
    def __init__(self, parent, items, flag):
        ...
        self.canvas = tk.Canvas(
            self, bg='white', height=600, width=500)  # Set canvas size
