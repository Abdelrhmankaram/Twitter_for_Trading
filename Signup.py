...
class SignupPage(tk.Frame):
    def __init__(self, parent, switch_to_login, switch_to_HomePage):
        ...
        tk.Label(self, image=self.img, bg='white').place(x=70, y=120)
        self.frame = tk.Frame(self, width=350, height=550, bg="white")
        self.frame.place(x=680, y=40)
        self.heading = tk.Label(self.frame, text="Signup", fg="#57A1F8", bg="white", font=(
        ...
        tk.Button(self.frame, font=("TkDefaultFont", 12), width=33, pady=7, text="Sign up",
                  bg='#57A1F8', fg="white", border=0, command=self.sign_up_command).place(x=32, y=y)
        y += 56

        label = tk.Label(self.frame, text="Already have an account?",
                         fg="black", bg="white", font=("Microsoft YaHei UI Light", 12))
        label.place(x=25, y=y)
        tk.Button(self.frame, width=6, text="Sign in", border=0, bg="white",
                  cursor='hand2', fg="#57A1F8", command=switch_to_login,font=("Microsoft YaHei UI Light", 12)).place(x=220, y=y)

    def create_entry(self, placeholder, y, on_enter, on_leave):
        entry = tk.Entry(self.frame, width=25, fg="black", border=0,
